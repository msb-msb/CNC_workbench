...
#
# derived from CADvas 
# A 2D CAD application written in Python 
# 

#
# Author: Doug Blanding   <doug dot blanding at localnet dot com>
# modifications by: Dan Falck </join #cam on freenode>
#
...
def arc_angle_format(direction,start_pt,cen_pt,end_pt):
    """Return arc in radius,center point, start angle, end angle format """
    #move the arc so that center is at origin for easy calcs
    new_start=sub_pt(start_pt,cen_pt)
    new_end  =sub_pt(end_pt,  cen_pt)
    start_angle= p2p_angle((0,0),new_start)
    end_angle= p2p_angle((0,0),new_end)
    #subtract angle1 from angle2
    angle_diff= (end_angle-start_angle)
    radius = p2p_dist(start_pt,cen_pt)
    if direction[2] == 1:
        new_arc = (radius,cen_pt,start_angle,end_angle)
    else:
        new_arc = (radius,cen_pt,end_angle,start_angle)
    return new_arc
...
def unitize(num):
    """ return -1 or 1 depending on angle directio """
